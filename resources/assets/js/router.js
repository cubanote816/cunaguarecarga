...
const routes = [
  ...
  {path: '/password/reset', component: require('./pages/auth/Reset.vue')},
  {path: '/resetmsg', component: require('./pages/auth/Resetmsg.vue')},
  ...
  {path: '/history', component: require('./pages/reports/History.vue'), meta: {requiresAuth: true}},
...
router.beforeEach((to, from, next) => {
  if (to.matched.some(record => record.meta.requiresAuth) && ! store.state.auth.me) {
    ...
    next('/login')
      ...
      ! includes(['admin', 'manager', 'reseller'], store.state.auth.me.role))) {
